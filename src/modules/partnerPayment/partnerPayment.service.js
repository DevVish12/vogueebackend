...
// const isVerifiedPartnerUpi = (partner) => {
//     const raw = partner?.upi_verified;
//     if (raw === true || raw === 1 || raw === '1') return true;
//     if (typeof raw === 'string') return ['true', '1'].includes(raw.trim().toLowerCase());
//     return false;
// };

// const enforceVerifiedUpi = (partner, message = 'Please complete and verify your UPI setup before going online.') => {
//     if (!isVerifiedPartnerUpi(partner)) {
...
//         const latestPartner = await PartnerAuthModel.findById(partnerId);
//         enforceVerifiedUpi(latestPartner, 'Please complete and verify your UPI setup before going online.');
...
//         return {
//             upi_id: partner.upi_id || null,
//             upi_verified: isVerifiedPartnerUpi(partner),
//             upi_verified_at: partner.upi_verified_at || null,
//         };
//     }

//     static async requireVerifiedUpi({ partnerId, jwtMobile }) {
//         const partner = await PartnerAuthModel.findById(partnerId);
//         if (!partner) {
//             const err = new Error('Partner not found');
//             err.statusCode = 404;
...
//         enforceVerifiedUpi(partner, 'Please complete and verify your UPI setup before going online.');
//         return partner;
//     }
// }
...
const PartnerPaymentModel = require('./partnerPayment.model');
const PartnerAuthModel = require('../partnerAuth/partnerAuth.model');
const { markPartnerAvailable, markPartnerUnavailable } = require('../../../socket/socket');
...
class PartnerPaymentService {
    ...
    static async getPresenceStatus({ partnerId, jwtMobile }) {
        const partner = await PartnerAuthModel.findById(partnerId);
        if (!partner) {
            const err = new Error('Partner not found');
            err.statusCode = 404;
        ...
        return {
            online_status: parseOnlineStatus(partner.online_status),
        };
    }

    static async setOnlineStatus({ partnerId, jwtMobile }) {
        const partner = await PartnerAuthModel.findById(partnerId);
        if (!partner) {
            const err = new Error('Partner not found');
            err.statusCode = 404;
        ...
        await db.query("UPDATE partners SET online_status = 'ONLINE' WHERE id = ?", [partnerId]);
        markPartnerAvailable(partnerId);
    ...
    static async setOfflineStatus({ partnerId, jwtMobile }) {
        const partner = await PartnerAuthModel.findById(partnerId);
        if (!partner) {
            const err = new Error('Partner not found');
            err.statusCode = 404;
        ...
        await db.query("UPDATE partners SET online_status = 'OFFLINE' WHERE id = ?", [partnerId]);
        markPartnerUnavailable(partnerId);

...
class AdminPartnerKycController {
  ...
  static async updateStatus(req, res, next) {
    try {
      ...
      const status = String(updated?.kyc_status || '').trim().toLowerCase();
      const notificationType = status === 'approved' || status === 'verified' ? 'KYC_APPROVED' : status === 'rejected' ? 'KYC_REJECTED' : null;
      if (notificationType && updated?.partner_id) {
        const [partnerRows] = await db.query('SELECT expo_push_token FROM partners WHERE id = ?', [updated.partner_id]);
        const token = partnerRows?.[0]?.expo_push_token;
        if (token) {
          void sendExpoNotification(
            token,
            notificationType === 'KYC_APPROVED' ? 'KYC Approved' : 'KYC Rejected',
            notificationType === 'KYC_APPROVED'
              ? 'Your partner verification has been approved.'
              : 'Your partner verification needs attention.',
            { screen: 'Profile' },
            {
              type: notificationType,
              eventId: `kyc_${updated.partner_id}_${notificationType}`,
              recipientId: updated.partner_id,

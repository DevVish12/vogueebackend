...
router.post('/mark-paid', adminProtect, async (req, res, next) => {
  ...
  try {
    ...
    // Push only after the payout rows have been marked paid successfully.
    try {
      const payoutKey = payoutId || payoutBatchId;
      const [partnerRows] = await db.query('SELECT expo_push_token FROM partners WHERE id = ?', [partnerId]);
      ...
      if (partnerToken) {
        void sendExpoNotification(
          ...
          { payoutId: payoutKey, screen: 'Earnings' },
          {
            type: 'PAYOUT_COMPLETED',
            eventId: `payout_${payoutKey}_COMPLETED`,
      ...
      console.warn('[NOTIFICATION] payout push lookup failed:', error?.message || 'unknown error');
    }

...
// const { sendExpoNotification } = require('../src/utils/sendExpoNotification');
...
// const emitPartnerPresenceUpdate = (reason, partnerId) => {
//     if (!io) return;
...
//     const sid = normalizeRoomId(socketId);
//     if (!pid || !sid) return;

//     const bucket = partnerSockets.get(pid) || new Set();
//     bucket.add(sid);
//     partnerSockets.set(pid, bucket);
// };
...
//     if (!isPartnerConnected(pid)) return;
//     if (onlinePartners.has(pid)) return;

//     onlinePartners.add(pid);
//     emitPartnerPresenceUpdate('partner_online', pid);
// };

// const markPartnerUnavailable = (partnerId) => {
//     const pid = normalizeRoomId(partnerId);
//     if (!pid) return;

//     if (!onlinePartners.has(pid)) return;

//     onlinePartners.delete(pid);
//     emitPartnerPresenceUpdate('partner_offline', pid);
// };

// const syncPartnerPresenceWithDb = async (partnerId) => {
//     const pid = normalizeRoomId(partnerId);
//     if (!pid) return;

//     if (!isPartnerConnected(pid)) {
//         markPartnerUnavailable(pid);
//         return;
//     }

//     try {
//         const partner = await PartnerAuthModel.findById(pid);
//         const status = String(partner?.online_status || '').trim().toUpperCase();

//         if (status === 'ONLINE') {
//             markPartnerAvailable(pid);
//             return;
//         }
//     } catch {
//         // ignore
...
//             syncPartnerPresenceWithDb(partnerId).catch(() => {
//                 // ignore
//             });
//         }
...
//         const registerPartnerRoom = async (rawPartnerId) => {
...
//             registerPartnerRoom(payload?.partnerId).catch(() => {
//                 // ignore
//             });
//         });
...
//                 try {
//                     const [userRows] = await db.query('SELECT expo_push_token FROM users WHERE id = ?', [updated.user_id]);
...
//                     }
//                 } catch (err) {
//                     console.warn('[socket] Failed to send booking cancellation push:', err?.message || err);
//                 }
...
//                         }
//                     } catch (err) {
//                         console.warn('[socket] Failed to send booking status push:', err?.message || err);
//                     }
//                 }
...
//             if (pid != null) {
//                 removePartnerSocket(pid, socket.id);
//                 if (!isPartnerConnected(pid)) {
//                     markPartnerUnavailable(pid);
//                 }
//             }
